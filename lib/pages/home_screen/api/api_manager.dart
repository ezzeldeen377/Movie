...
import 'package:movie/pages/home_screen/model/recommended_response.dart';
...
class ApiManager {
  ...
  static Future<NewsReleases?> getNewsReleases() async {
    ...
    try {
      var response = await http.get(url);
      if (response.statusCode == 200) {
        return NewsReleases.fromJson(jsonDecode(response.body));
      } else {
        throw Exception(
  ...
  static Future<NewsReleases?> getMoreLikeThis(String id) async {
